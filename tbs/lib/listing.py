...
class Listing:
    ...
    def validate_id(id: Union[int, str]) -> Optional[int]:
        """Returns validated ID or `None`, if ID is incorrect."""
    ...
    def validate(self,
                 ...
                 ) -> Tuple[Optional[int], Direction, int]:
        ...
        if before is not None:
            return self.validate_id(before), Direction.BEFORE, limit
        if after is not None:
            return self.validate_id(after), Direction.AFTER, limit

        return None, Direction.AFTER, limit
